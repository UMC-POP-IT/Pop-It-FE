 ...
 *
 * ⚠️ CalendarMonthGrid와 겹치는 로직 — 한쪽을 고치면 다른 쪽도 확인할 것
 * ---------------------------------------------------------------------
 * 크기 값은 전부 다르지만 아래 '판단 순서'는 두 파일이 같다. 달력 동작 버그를
 * 고칠 때는 두 파일을 같이 열어야 한다.
 *   - renderDayNumber 우선순위: 선택됨 > 오늘 > 그 외 선택 불가 > 기본
 *     (오늘은 대부분 isDisabled에도 걸리므로 반드시 disabled보다 먼저 검사)
 *   - cells에 null이 오면 빈 칸을 그리고 aria-hidden 처리
 *   - 월 이동 화살표는 갈 수 없는 방향이면 버튼을 그리지 않되 자리(h-8 w-8)는 남김
 *   - 날짜 버튼의 disabled / aria-label / aria-pressed 연결 방식
 * 장기적으로는 크기·모드를 prop으로 받아 한 컴포넌트로 합치는 게 맞다. 다만 그러려면
 * 칸 높이·가로폭(breakpoint별)·원 크기 방식(고정 px냐 칸 높이 채우기냐)·띠 방식
 * (별도 absolute span이냐 칸 배경이냐)·요일줄 높이/색/크기·카드 패딩과 폭까지
 * 8개 가까운 prop이 필요하고, 그건 공용 컴포넌트가 "prop으로 받지 않는다"고
 * 못 박은 지점을 되돌리는 일이라 그 파일 담당자와 함께 결정해야 한다.
 *
